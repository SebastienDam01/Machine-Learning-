...
# la 10ème parabole. Cela signifie que ça a été obtenu avec le sous ensemble de
# 10 prédicteurs. Voici le sous ensemble de predicteurs en question :

columns <- coef(leaps, 10)
columns.names <- names(columns)
columns.names
#"season", "mnth", "holiday", "weekday", "workingday", "weathersit", "temp"        "atemp"       "hum"        "windspeed"  

# Nous savons maintenant que ces 10 prédicteurs permettent d'obtenir le MSE
...
svm.radial <- train(
  cnt~., data = bike, method = "svmRadial",
  preProcess = c("center","scale"),
  trControl = train.control,
  tuneLength = 10)
