...
# La courbe est presque une droite, ce qui porte à croire que dans ce sous ensemble
# de variables, chaqune contribue à la variance et donc on ne pourra pas
# réduire la dimension ultérieurement
...
models <- regsubsets(cnt~., data = bike, nvmax = 9)
summary(models)
...
# sur chaque modèle, pour chaque modèle elle teste les 9 sous ensembles possibles 
# (de 1 à 9 prédicteurs) tout en testant pour chaque combinaison, 
...
min.MSE.svm.radial
# svm radial lowest MSE         : 299 576

# Dans ce plot nous pouvons voir un peu mieux comment la boucle au dessus procède,
# on teste le modèle avec 9 sous ensembles de prédicteurs (de 1 à 9) et pour
# chaque sous ensemble on teste 10 valeurs du parametre spécifique au modèle
# dans le cas du svm avec radial kernel c'est le C (cost). On a donc fait 90
# cross validations pour aboutir au meilleur paramétrage du modèle. La métrique 
# utilisée est toujours le RMSE. On voit qu'il y a 9 paraboles 
...
# la 5ème parabole. Cela signifie que ça a été obtenu avec le sous ensemble de
# 5 prédicteurs. Voici le sous ensemble de predicteurs en question :

columns <- coef(leaps, 5)
columns.names <- names(columns)
columns.names
# "day", "mnth", "weathersit","temp" , "windspeed"  

# Nous savons maintenant que ces 5 prédicteurs permettent d'obtenir le MSE
...
svm.radial <- train(
  cnt~mnth+weathersit+temp+windspeed+day, data = bike, method = "svmRadial",
  preProcess = c("center","scale"),
  trControl = train.control,
  tuneLength = 10)
...
set.seed(1729) # Hardy-Ramanujan number
