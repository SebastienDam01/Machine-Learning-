...
pca <- princomp(scale(phonemes[, 1:256]))
U <- pca$loadings
phonemes_pc <- cbind.data.frame(pca$scores[,1:50], phonemes$y)
names(phonemes_pc)[ncol(phonemes_pc)] <- "y"
...
CV <- rep(0,10)
#Creating folds
fold <- unname(createFolds(phonemes$y, k=10))
for(i in (1:10)){
  
  #Training data
  train_data <- phonemes[-fold[[i]], ]
  #Creating test data 
  test_data <- phonemes[fold[[i]], ]
...
phonemes$y=as.numeric(factor(phonemes$y))

## reassign values to y from 0 to 4 to fit multinom condition on gam
phonemes$y[phonemes$y==1] <- 0
phonemes$y[phonemes$y==2] <- 1
phonemes$y[phonemes$y==3] <- 2
phonemes$y[phonemes$y==4] <- 3
phonemes$y[phonemes$y==5] <- 4
