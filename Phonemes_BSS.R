...
errors_BSS <-  matrix(0, 10, 12)
colnames(errors_BSS) <- c("NB", "Knn", "LDA", "QDA", "FDA", "RL", "Ridge", "Lasso", "ElasticNet", "Bagging", "RF", "SVM")

## FSS 

library(leaps)

reg.backward<-regsubsets(as.numeric(factor(y))~., data=phonemes,method='backward',nvmax=100)
plot(reg.backward,scale="bic")

res.backward <- summary(reg.backward)

best <- which.min(res.backward$bic)

best_coef = coef(reg.backward,best)
best_coef

phonemes_BSS <- phonemes[,res.backward$which[best,]]
phonemes_BSS$y = phonemes$y

# K Fold cross validation on Naive Bayes

CV <- rep(0,10)
#Creating folds
fold <- unname(createFolds(phonemes_BSS$y, k=10))
for(i in (1:10)){
  #Training data
  train_data <- phonemes_BSS[-fold[[i]], ]
  #Creating test data 
  test_data <- phonemes_BSS[fold[[i]], ]
  
  nb.phonemes_BSS <- naive_bayes(y~., data=train_data)
  pred.phonemes_BSS <- predict(nb.phonemes_BSS, newdata=test_data)
  
  perf <- table(test_data$y, pred.phonemes_BSS)
  
  CV[i]<-1-sum(diag(perf))/nrow(test_data) 
  errors_BSS[i, c("NB")] <- CV[i]
}
...
phonemes_BSS$y <- as.numeric(factor(phonemes_BSS$y))
Kmax <- 25
...
folds.outer <- createFolds(phonemes_BSS$y, k=10)
...
for(outer.fold in folds.outer){
  ...
  data.outer.test <- phonemes_BSS[outer.fold.indexes, ]
  ...
  data.outer.train <- phonemes_BSS[outer.folds.indexes, ]
  ...
  for (inner.fold in folds.inner){
    ...
    for(k in K.sequences){
      model <- knn(train=data.inner.train[,1:40], 
                   test = data.inner.val[,1:40], 
...
for(i in (1:10)){
  ...
  lda.phonemes_BSS <- lda(y~., data=train_data)
  pred.phonemes_BSS <- predict(lda.phonemes_BSS, newdata=test_data)
  
  perf <- table(test_data$y, pred.phonemes_BSS$class)
  
  CV[i]<-1-sum(diag(perf))/nrow(test_data) 
  errors_BSS[i, c("LDA")] <- CV[i]
}

# http://statweb.lsu.edu/faculty/li/teach/exst7152/phoneme-example.html
plot(pred.phonemes_BSS$x,xlab="Dim 1",ylab="Dim 2",col=col[fold[[10]]],pch=col[fold[[10]]],cex=0.7)
...
for(i in (1:10)){
  ...
  qda.phonemes_BSS <- qda(y~., data=train_data)
  pred.phonemes_BSS <- predict(qda.phonemes_BSS, newdata=test_data)
  
  perf <- table(test_data$y, pred.phonemes_BSS$class)
  
  CV[i]<-1-sum(diag(perf))/nrow(test_data) 
  errors_BSS[i, c("QDA")] <- CV[i]
}
...
for(i in (1:10)){
  ...
  fda.phonemes_BSS <- fda(y~., data=train_data)
  pred.phonemes_BSS <- predict(fda.phonemes_BSS, newdata=test_data)
  
  perf <- table(test_data$y, pred.phonemes_BSS)
  
  CV[i]<-1-sum(diag(perf))/nrow(test_data)
  errors_BSS[i, c("FDA")] <- CV[i]
}
...
for(i in (1:10)){
  ...
  rl.phonemes_BSS <- nnet::multinom(y~., data=train_data)
  pred.phonemes_BSS <- predict(rl.phonemes_BSS, newdata=test_data)
  
  perf <- table(test_data$y, pred.phonemes_BSS)
  
  CV[i]<-1-sum(diag(perf))/nrow(test_data)
  errors_BSS[i, c("RL")] <- CV[i]
}
...
for(i in (1:10)){
  ...
  x <- as.matrix(train_data[,1:40])
  y <- as.factor(train_data$y)
  
  newX <- model.matrix(~.-y,data=test_data)
  
  X.test <- as.matrix(test_data[,1:40])
  
  ridge.phonemes_BSS <- cv.glmnet(x, y, alpha=0, family='multinomial', nfold=3)
  best.lambda = ridge.phonemes_BSS$lambda.min
  
  pred.phonemes_BSS <- predict(ridge.phonemes_BSS, newx=X.test, s="lambda.min", type="class")
  
  CV[i]<-length(which(pred.phonemes_BSS!=test_data$y))/nrow(X.test)
  errors_BSS[i, c("Ridge")] <- CV[i]
}
...
for(i in (1:10)){
  ...
  lasso.phonemes_BSS <- cv.glmnet(x, y, alpha=1, family='multinomial', nfold=3)
  best.lambda = lasso.phonemes_BSS$lambda.min
  
  pred.phonemes_BSS <- predict(lasso.phonemes_BSS, newx=X.test, s="lambda.min", type="class")
  
  CV[i]<-length(which(pred.phonemes_BSS!=test_data$y))/nrow(X.test)
  errors_BSS[i, c("Lasso")] <- CV[i]
}
...
for(i in (1:10)){
  ...
  X.train <- train_data[,1:40]
  y.train <- train_data[, 41]
  X.test <- as.matrix(test_data[,1:40])
  y.test <- test_data[, 41]
  
  elastic_net.phonemes_BSS <- glmnet(X.train, y.train, alpha=best.alpha, lambda=best.lambda, family='multinomial', nfold=3)
  
  pred.phonemes_BSS <- predict(elastic_net.phonemes_BSS, X.test, type="class")
  
  CV[i]<-1-sum(diag(table(y.test,pred.phonemes_BSS)))/nrow(X.test)
  errors_BSS[i, c("ElasticNet")] <- CV[i]
}

# KFold cross validation on Bagging

CV <- rep(0,10)
p <- ncol(phonemes_BSS) - 1
#Creating folds
fold <- unname(createFolds(phonemes_BSS$y, k=10))
for(i in (1:10)){
  #Training data
  ...
  bag.phonemes_BSS <- randomForest(as.factor(y) ~ ., train_data, mtry=p)
  pred.phonemes_BSS <- predict(bag.phonemes_BSS, newdata=test_data, type="response")
  
  CV[i]<-1-sum(diag(table(test_data$y,pred.phonemes_BSS)))/nrow(test_data)
  errors_BSS[i, c("Bagging")] <- CV[i]
}
...
for(outer.fold in folds.outer){
  ...
  model.mse.ave[i] <- errc
  errors_BSS[i, c("RF")] <- model.mse.ave[i]
}
...
for(i in 1:N){
  CV[i]<-cross(
    ksvm(as.factor(y)~.,data=phonemes_BSS,type="C-svc",kernel="rbfdot",C=C_list[i],cross=10)
  )
}
...
for(i in (1:10)){
  ...
  svm.phonemes_BSS <-ksvm(as.factor(y)~ .,data=train_data,type="C-svc",kernel="rbfdot",C=1)
  pred.phonemes_BSS=predict(svm.phonemes_BSS,test_data[1:40])
  
  CV[i]<-1 - sum(diag(table(test_data$y, pred.phonemes_BSS)))/nrow(test_data)
  errors_BSS[i, c("SVM")] <- CV[i]
}
...
plot.cv.error <- function(data, x.title="x"){
  ...
  ggplot(data.frame(model=x.title, mean=as.vector(means.errs)),aes(x=model, y=mean))+
    ...
    ggtitle("Intervalle de confiance des erreurs en fonction du prédicteur BSS") +
    xlab("Prédicteurs")+
...
plot.cv.error(errors_BSS, c("Naive Bayes", "KNN", "LDA", "QDA", "FDA", "RL", "Ridge", "Lasso", "ElasticNet", "Bagging", "RF", "SVM"))

#boxplot(errors_BSS,main="Boîtes à moustache des erreurs selon le prédicteur", xlab='Prédicteurs', ylab="Pourcentage d'erreur")